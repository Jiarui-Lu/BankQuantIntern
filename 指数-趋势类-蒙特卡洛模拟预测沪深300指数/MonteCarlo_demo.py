...
def plot(df, forecast_horizon, d, pick):
    ...
    plt.ylabel('Price')
    plt.xlabel('Date')
    plt.savefig(r'result\simulation.jpg')
    plt.show()
    ...
    plt.ylabel('Price')
    plt.xlabel('T+Days')
    plt.savefig(r'result\training_versus_testing.jpg')
    plt.show()
...
def test(df, simu_start=100, simu_end=500, simu_delta=100, **kwargs):
    table = pd.DataFrame()
    ...
    plt.savefig(r'result\prediction_accuracy.jpg')
    plt.show()
...
def main():
    stdate = '2016-01-15'
    eddate = '2019-01-15'
    df = pd.read_excel(r'data\HS300data.xlsx', index_col=0)
    # print(df)
