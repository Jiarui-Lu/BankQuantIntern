...
def plot(signals, close_price):
    ...
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.savefig(r'result\positions.jpg')
    plt.show()
...
def profit(portfolio, close_price):
    ...
    plt.savefig(r'result\Total Assets.jpg')
    plt.show()
...
def main():
    df = pd.read_csv(r'data\brent crude nokjpy.csv')
    signals = signal_generation(df, 'brent', 'nok', oil_money)
    # print(signals[signals.columns[0]])
    p = portfolio(signals, 'nok')
    signals.index = signals[signals.columns[0]]
    del signals[signals.columns[0]]
    # print(signals)
