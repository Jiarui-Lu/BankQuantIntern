...
def plot(df):
    ...
    plt.grid(True)
    plt.legend(loc='best')
    plt.savefig(r'result\HA_strategy.jpg')
    plt.show()
...
def portfolio(data, capital0=10000, positions=100):
    ...
    portfolio['date'] = data[data.columns[0]]
...
def profit(portfolio):
    ...
    plt.savefig(r'result\Positions_total_assets.jpg')
    plt.show()
...
# stats calculation
def stats(portfolio, trading_signals, capital0=10000):
    ...
    # use DJI as benchmark
    benchmark = pd.read_excel(r'data\DJI.xls', index_col=0)
    ...
    stats['maximum drawdown'] = mdd(list(portfolio['total asset']))
...
def main():
    ...
    df = pd.read_excel(r'data\NVDA.xls', index_col=0)
    ...
    s = stats(portfolio_details, trading_signals)

    s.to_excel(r'result\stats.xls')
